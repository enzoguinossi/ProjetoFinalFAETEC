...
export class CondutorDAO {
  ...
  async create(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      await tx.registroAuditoria.create({
        data: { id_usuario, acao: "CRIAR", entidade: "Condutor", id_entidade_afetada: c.id_condutor, dados_novos: { id_funcionario: data.id_funcionario } },
      });
      return c;
    });
  }
  ...
  async softDelete(id: number, id_usuario: number) {
    return prisma.$transaction(async (tx) => {
      ...
      await tx.registroAuditoria.create({
        data: { id_usuario, acao: "DESATIVAR", entidade: "Condutor", id_entidade_afetada: id, dados_anteriores: { ativo: antes.ativo }, dados_novos: { ativo: depois.ativo } },
      });
      return depois;
    });
  }
